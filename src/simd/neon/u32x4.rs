...
impl SimdVec<u32> for U32x4 {
    ...
    fn splat(value: u32) -> Self {
        Self {
            elements: unsafe { vdupq_n_u32(value) },
            size: LANE_COUNT,
        }
    }
    ...
    unsafe fn load_partial(ptr: *const u32, size: usize) -> Self {
        ...
        let mut elements = vdupq_n_u32(0);

        // Load elements individually using vsetq_lane
        match size {
            1 => {
                elements = vsetq_lane_u32(*ptr.add(0), elements, 0);
            }
            2 => {
                elements = vsetq_lane_u32(*ptr.add(0), elements, 0);
                elements = vsetq_lane_u32(*ptr.add(1), elements, 1);
            }
            3 => {
                elements = vsetq_lane_u32(*ptr.add(0), elements, 0);
                elements = vsetq_lane_u32(*ptr.add(1), elements, 1);
                elements = vsetq_lane_u32(*ptr.add(2), elements, 2);
            }
    ...
    fn store(&self) -> Vec<u32> {
        ...
        unsafe {
            vst1q_u32(vec.as_mut_ptr(), self.elements);
        }
    ...
    unsafe fn store_at_partial(&self, ptr: *mut u32) {
        ...
        match self.size {
            1 => *ptr.add(0) = vgetq_lane_u32(self.elements, 0),
            2 => {
                *ptr.add(0) = vgetq_lane_u32(self.elements, 0);
                *ptr.add(1) = vgetq_lane_u32(self.elements, 1);
            }
            3 => {
                *ptr.add(0) = vgetq_lane_u32(self.elements, 0);
                *ptr.add(1) = vgetq_lane_u32(self.elements, 1);
                *ptr.add(2) = vgetq_lane_u32(self.elements, 2);
            }
            _ => {
                let msg = "WTF is happening here";
                panic!("{}", msg);
    ...
    fn eq_elements(&self, rhs: Self) -> Self {
        ...
        let elements = unsafe { vceqq_u32(self.elements, rhs.elements) };
    ...
    fn lt_elements(&self, rhs: Self) -> Self {
        ...
        // Compare a<b elementwise
        let elements = unsafe { vcltq_u32(self.elements, rhs.elements) };
    ...
    fn le_elements(&self, rhs: Self) -> Self {
        ...
        // Compare a<=b elementwise
        let elements = unsafe { vcleq_u32(self.elements, rhs.elements) };
    ...
    fn gt_elements(&self, rhs: Self) -> Self {
        ...
        // Compare a>b elementwise
        let elements = unsafe { vcgtq_u32(self.elements, rhs.elements) };
    ...
    fn ge_elements(&self, rhs: Self) -> Self {
        ...
        // Compare a>=b elementwise
        let elements = unsafe { vcgeq_u32(self.elements, rhs.elements) };
...
impl Add for U32x4 {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        ...
        unsafe {
            U32x4 {
                size: self.size,
                elements: vaddq_u32(self.elements, rhs.elements),
            }
        }
    }
}
...
impl Sub for U32x4 {
    ...
    fn sub(self, rhs: Self) -> Self::Output {
        ...
        unsafe {
            U32x4 {
                size: self.size,
                elements: vsubq_u32(self.elements, rhs.elements),
            }
        }
    }
}
...
impl Mul for U32x4 {
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        ...
        unsafe {
            U32x4 {
                size: self.size,
                elements: vmulq_u32(self.elements, rhs.elements),
            }
        }
    }
}
...
impl PartialEq for U32x4 {
    fn eq(&self, other: &Self) -> bool {
        ...
        unsafe {
            ...
            let cmp = vceqq_u32(self.elements, other.elements);

            // Reinterpret result as float for mask extraction
            // let mask = vget_lane_u32(vmovn_u64(vreinterpretq_u64_u32(vreinterpretq_u32_u8(cmp))), 0);
...
impl PartialOrd for U32x4 {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        ...
        unsafe {
            ...
            let lt_mask = vandq_u32(lt, vdupq_n_u32(0x1));
            let gt_mask = vandq_u32(gt, vdupq_n_u32(0x1));
            let eq_mask = vandq_u32(eq, vdupq_n_u32(0x1));
...
impl BitAnd for U32x4 {
    ...
    fn bitand(self, rhs: Self) -> Self::Output {
        ...
        let elements = unsafe { vandq_u32(self.elements, rhs.elements) };
...
impl BitOr for U32x4 {
    ...
    fn bitor(self, rhs: Self) -> Self::Output {
        ...
        let elements = unsafe { vorrq_u32(self.elements, rhs.elements) };
...
mod u32x4_tests {
    ...
    fn test_add() {
        ...
        let v2 = U32x4::new(&[1, 10]);
        let u2 = U32x4::new(&[5, 11]);

        assert_eq!(vec![6, 21], (u2 + v2).to_vec());

        let v3 = U32x4::new(&[1, 10, 7]);
        let u3 = U32x4::new(&[5, 11, 9]);

        assert_eq!(vec![6, 21, 16], (u3 + v3).to_vec());

        let v4 = U32x4::new(&[1, 10, 7, 2]);
        let u4 = U32x4::new(&[5, 11, 9, 5]);

        assert_eq!(vec![6, 21, 16, 7], (u4 + v4).to_vec());

        let v5 = U32x4::new(&[1, 10, 7, 2]);
        let u5 = U32x4::new(&[5, 11, 9, 5]);
    ...
    fn test_sub() {
        ...
        let v5 = U32x4::new(&[1, 10, 7, 2, 1]);
        let u5 = U32x4::new(&[5, 11, 9, 5, 1]);
    ...
    #[test]
    fn test_sub_assign() {
        let mut a = U32x4::new(&[4, 2, 3, 4]);
        let b = U32x4::new(&[1, 2, 2, 1]);
