...
impl SimdVec<i64> for I64x4 {
    ...
    fn splat(value: i64) -> Self {
        Self {
            elements: unsafe { _mm256_set1_epi64x(value) },
            size: LANE_COUNT,
        }
    }
    ...
    unsafe fn load_partial(ptr: *const i64, size: usize) -> Self {
        ...
        let elements = match size {
            1 => unsafe { _mm256_set_epi64x(0, 0, 0, *ptr.add(0)) },
            2 => unsafe { _mm256_set_epi64x(0, 0, *ptr.add(1), *ptr.add(0)) },
            3 => unsafe { _mm256_set_epi64x(0, *ptr.add(2), *ptr.add(1), *ptr.add(0)) },
            _ => {
    ...
    unsafe fn store_at_partial(&self, ptr: *mut i64) {
        ...
        unsafe {
            _mm256_maskstore_epi64(ptr, mask, self.elements);
        }
    }
...
impl Mul for I64x4 {
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        ...
        let low_low = unsafe { _mm256_mul_epu32(self.elements, rhs.elements) }; // a_lo * b_lo

        let self_hi = unsafe { _mm256_srli_epi64(self.elements, 32) };
        let rhs_hi = unsafe { _mm256_srli_epi64(rhs.elements, 32) };

        let high_low = unsafe { _mm256_mul_epu32(self_hi, rhs.elements) };

        let low_high = unsafe { _mm256_mul_epu32(self.elements, rhs_hi) };

        let cross_terms = unsafe { _mm256_add_epi64(high_low, low_high) };
        let cross_shifted = unsafe { _mm256_slli_epi64(cross_terms, 32) };

        let elements = unsafe { _mm256_add_epi64(low_low, cross_shifted) };
...
mod i64x4_tests {
    ...
    fn test_add() {
        ...
        let v2 = I64x4::new(&[1, 10]);
        let u2 = I64x4::new(&[5, 11]);
    ...
    #[test]
    fn test_add_assign() {
        let mut a = I64x4::new(&[1, 2, 3, 4]);
        let b = I64x4::new(&[4, 3, 2, 1]);
